...
class SymphonyParser(object):
  ...
  def add_subparser(self, name, aliases=None, **kwargs):
    ...
    if not hasattr(self, method_name):
      raise ValueError('your parser class must define the method "{}()" '
                       'in order to use the "{}" subparser'.format(
                           method_name, name))
  ...
  def _setup_scp(self):
    ...
    parser.add_argument(
        'src_file',
        help=
        'source file or folder. "[[<process_group>/]<process>]:/file/path" denotes remote.'
    )
    parser.add_argument(
        'dest_file',
        help=
        'destination file or folder. "[[<process_group>/]<process>]:/file/path" denotes remote.'
    )
    self._add_experiment_name(parser, required=False, positional=True)
    self.add_dry_run(parser)
  ...
  def _setup_list_experiments(self):
    parser = self.add_subparser('list-experiments', aliases=['ls', 'lse'])
    # no arg to get the current namespace
    self._add_experiment_name(parser, required=False, positional=True)

  def _setup_switch_experiment(self):
    parser = self.add_subparser('switch-experiment', aliases=['e', 'se'])
    parser.add_argument(
        '-f',
        '--force',
        action='store_true',
  ...
  def _setup_list_processes(self):
    """
        ...
        """
    parser = self.add_subparser('list-processes', aliases=['lsp', 'p'])
    self._add_experiment_name(parser, required=False, positional=True)

  def _setup_visit(self):
    parser = self.add_subparser('visit', aliases=['vi'])
    parser.add_argument('service_name',
                        help='the name of the service to visit')
  ...
  def _add_experiment_name(self, parser, required=True, positional=True):
    ...
    if positional and required:
      parser.add_argument('experiment_name', nargs=None, help=help_str)
  ...
  def _add_component_arg(self, parser):
    parser.add_argument('component_name', help="[<process_group>/]<process>")
  ...
  def _interactive_find_process(self, proc_path, exp_name):
    ...
    if '/' in proc_path:  # must be exact match
      ...
      proc_pairs, is_exact = self.cluster.fuzzy_match_process(
          proc_name, exp_name)
      if is_exact:
  ...
  def _interactive_find(self, matches, error_message):
    """
        ...
        """
    if len(matches) == 0:
      print_err('[Error] {} not found. Please check for typos.'.format(
          error_message))
      sys.exit(1)
    elif len(matches) == 1:
      match = matches[0]
      print_err(
          '[Warning] No exact match. Fuzzy match only finds one candidate: {}'.
          format(match))
      return match
    ...
    try:
      ...
      sys.exit(1)
    if ans >= len(matches):
      print_err('[Error] Must enter a number between 0 - {}'.format(
          len(matches) - 1))
      sys.exit(1)
    return matches[ans]
  ...
  def _print_experiment(self, data, min_width=5, max_width=1000, pad=2):
    ...
    for key in columns:
      ...
      width[key] = w
    row_format = ''.join(
        ['{{:<{}}}'.format(width[header]) for header in headers])
  ...
  def action_log(self, args):
    ...
    self.cluster.get_log(experiment_name=experiment_name,
  ...
  def action_exec(self, args):
    ...
    self.cluster.exec_command(process_name=process_name,
  ...
  def action_scp(self, args):
    ...
    self.cluster.transfer_file(exp,
                               src_path,
                               dest_path,
                               src_process=src_p,
  ...
  def action_ssh(self, args):
    ...
    self.cluster.login(experiment_name=exp,
                       process_name=p,
                       process_group_name=pg)

  def action_visit(self, args):
    url = self.cluster.external_url(self._get_experiment(args),
                                    args.service_name)
    if url:
      url = 'http://' + url
      print(url)
